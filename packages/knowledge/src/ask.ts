...
export function buildAskPrompt(map: RepoMap, prose: string, question: string, drift: KnowledgeDriftReport): string {
  const lines = [
    ...
    "",
    "Output contract:",
    "- Your reply IS the answer. Write the full answer as the text of this reply, inline. Whatever is not in your reply is lost — there is no follow-up turn and no reader on the other side.",
    "- Do not defer any part of the answer to a file, and do not write, edit, or reference a separate document or \"plan file\" to hand it off. There is none.",
    "- Do not reach for a plan or hand-off tool (e.g. ExitPlanMode) and do not close with a meta-remark about the answer being \"above\" or \"complete\" — just produce the answer itself as your final message.",
    "",
