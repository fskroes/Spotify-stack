...
| [`2026-08-05-step-1-requirements-named.md`](2026-08-05-step-1-requirements-named.md) | The panel's step-1 close: the gate-2 row count rewritten from "tens" to a signed N=30 (rule of three, 10% detectable unreviewed rate), and all 21 ADRs re-chosen by name — 12 KEEP, 9 QUESTION. Nothing merged or deleted here; that is the next step, taken only from this recorded list. |
...
| [`experiments/2026-08-07-the-ten-percent-sweep.md`](experiments/2026-08-07-the-ten-percent-sweep.md) | Every part left after three deletions, each with a named requester and a last real use. Two cut ([ADR-0029](adr/0029-the-in-flight-store-is-deleted.md), [ADR-0030](adr/0030-the-vetoed-status-leaves-the-vocabulary.md)), four kept with the reason recorded — because an unrecorded keep is indistinguishable from never having looked. Finds that ADR-0025 kept the `vetoed` status on a claim about the archive that the archive falsifies: 0 rows, not one. |
